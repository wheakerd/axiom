...
def external_current_observation() -> dict:
    cases = corpus_cases()
    result_cases = []
...
class RoutingEvaluationTests(unittest.TestCase):
    def test_checked_in_evaluation_contracts_pass(self):
        failures: list[str] = []
        self.assertEqual((115, 50, 11), check_routing_evaluations(failures))
    ...
    def test_external_post_tag_observation_rejects_partial_or_candidate_evidence(self):
        ...
        candidate = external_current_observation()
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            path = write_content_addressed_observation(
                Path(temporary_directory), external_current_observation()
            )
            failures = []
            validate_external_routing_observation(
                path,
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            payload = (json.dumps(external_current_observation(), indent=2) + "\n").replace(
                '  "schemaVersion": "2",',
                '  "schemaVersion": "2",\n  "schemaVersion": "2",',
                1,
            ).encode("ascii")
