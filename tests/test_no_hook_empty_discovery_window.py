...
class EmptyDiscoveryWindowTests(unittest.TestCase):
    ...
    def prepared(self, *, outcome_semantics=False, accepted_remainder=False, query_error=False):
        ...
        def synthetic_registration(path, *args, **kwargs):
            ...
            if path in (ROOT / native.HISTORY_RELATIVE, ROOT / replies.HISTORY):
                ...
                key = 'acceptedRemainder' if accepted_remainder else 'discoveryOutcomeAcceptance' if outcome_semantics else 'nativeEmptyDiscoveryAcceptance'
                if key in history:
                    history[key]['results'] = []
        ...
        def query(paths, executable, marketplace, installed, definition):
            ...
            return receipt
        self.query = query
        contract = native.ACCEPTED_REMAINDER if accepted_remainder else native.OUTCOME_ACCEPTANCE if outcome_semantics else native.EMPTY_DISCOVERY_ACCEPTANCE
        ...
        with patch.object(native, '_revision_four_auth_source', return_value=old), \
            ...
            native.prepare_fixed_acceptance(ROOT, run, previous, self.parent/'bundle',
                ...
                empty_discovery=not (outcome_semantics or accepted_remainder), outcome_semantics=outcome_semantics, accepted_remainder=accepted_remainder, runner=runner)
