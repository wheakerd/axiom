...
The immutable v0.7.9 gate is the cumulative baseline for the v0.8.6 candidate:

| Metric | Baseline | v0.8.6 candidate | Delta | Classification |
| --- | ---: | ---: | ---: | --- |
...
The v0.8.6 record represents all required paths: fresh startup with a no-route
...
v0.8.6 is `NOT-RUN`; authenticated Claude Code observation is
...
Prior v0.8.5 observations remain external evidence and are not copied into the
v0.8.6 candidate's host metrics. Current exact host usage is therefore
`NOT-RUN`; the deterministic static measurement is local and telemetry-free,
...
versioned record is [v0.8.6](results/v0.8.6.json). The v0.8.5 record remains
