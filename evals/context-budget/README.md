...
The immutable v0.7.9 gate is the cumulative baseline for the v0.8.14 candidate:

| Metric | Baseline | v0.8.14 candidate | Delta | Classification |
| --- | ---: | ---: | ---: | --- |
...
The v0.8.14 record represents all required paths: fresh startup with a no-route
...
the new observable-refusal and independent-audit `review-axiom-task` contracts,
...
v0.8.14 is `NOT-RUN`; authenticated Claude Code observation is
...
The immutable v0.8.13 release and prior observations remain separate evidence and
are not copied into the v0.8.14 candidate's host metrics. Current exact host
...
SemVer record; v0.7.9 remains only the cumulative growth baseline. The v0.8.14
candidate grows from v0.8.13, so its current reduction experiment is correctly
null. The most recent actual reduction remains the historical
...
versioned record is [v0.8.14](results/v0.8.14.json). The v0.8.13 record remains
byte-for-byte historical evidence; the current record binds the corrected gate,
