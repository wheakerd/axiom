...
The immutable v0.7.9 gate is the cumulative baseline for the v0.8.17 candidate:

| Metric | Baseline | v0.8.17 candidate | Delta | Classification |
| --- | ---: | ---: | ---: | --- |
...
The v0.8.17 record represents all required paths: fresh startup with a no-route
...
v0.8.17 is `NOT-RUN`; authenticated Claude Code observation is
...
The immutable v0.8.16 release and prior observations remain separate evidence and
are not copied into the v0.8.17 candidate's host metrics. Current exact host
...
SemVer record; v0.7.9 remains only the cumulative growth baseline. The v0.8.17
candidate is byte-identical to v0.8.16, so its current reduction experiment is
...
versioned record is [v0.8.17](results/v0.8.17.json). The v0.8.16 record remains
byte-for-byte historical evidence; the current record binds the unchanged gate,
fixed workload, governance-policy patch, and static-only evidence boundary.
