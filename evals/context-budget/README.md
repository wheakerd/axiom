...
The immutable v0.7.9 gate is the cumulative baseline for the v0.8.15 candidate:

| Metric | Baseline | v0.8.15 candidate | Delta | Classification |
| --- | ---: | ---: | ---: | --- |
...
The v0.8.15 record represents all required paths: fresh startup with a no-route
...
v0.8.15 is `NOT-RUN`; authenticated Claude Code observation is
...
The immutable v0.8.14 release and prior observations remain separate evidence and
are not copied into the v0.8.15 candidate's host metrics. Current exact host
...
SemVer record; v0.7.9 remains only the cumulative growth baseline. The v0.8.15
candidate is byte-identical to v0.8.14, so its current reduction experiment is
...
versioned record is [v0.8.15](results/v0.8.15.json). The v0.8.14 record remains
byte-for-byte historical evidence; the current record binds the unchanged gate,
fixed workload, Windows wrapper change, and static-only evidence boundary.
