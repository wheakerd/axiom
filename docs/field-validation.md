...
For the v0.8.9 candidate, a final Stage 3 result belongs outside the checked-in
...
python3 scripts/check-publication.py \
  ...
  /absolute/path/axiom-v0.8.9-codex-core-v2-<full-sha256>.json \
...
The subject must be non-candidate v0.8.9 with a non-null `v0.8.9` tag and the
...
3. If the batch passes, create immutable tag `v0.8.9` at that same commit.
...
5. Create one draft `Axiom v0.8.9` Release targeting the exact 40-character
...
6. With an owner credential, read the repository immutable-release setting and
   ...
   exact `v0.8.9` ref with only `tag=v0.8.9`. The workflow requires the tag
...
7. Explicitly dispatch `Release signature guard` on `v0.8.9` and
...
8. Close Issue #59 only after the signature guard, publication workflow,
